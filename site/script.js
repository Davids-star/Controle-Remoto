...
// Variáveis para acumular o movimento e enviar em intervalos (Batching)
let accumulatedDX = 0;
let accumulatedDY = 0;
let mouseUpdateTimer = null;

function sendMouseMovement() {
    if (accumulatedDX !== 0 || accumulatedDY !== 0) {
        mouseMoverRelativo(accumulatedDX, accumulatedDY);
        accumulatedDX = 0;
        accumulatedDY = 0;
    }
}
...
area.addEventListener("touchstart", e => {
    ...
    if (!mouseUpdateTimer) {
        mouseUpdateTimer = setInterval(sendMouseMovement, 20); // 50hz
    }
...
area.addEventListener("touchmove", e => {
    ...
    // Acumula o movimento em vez de enviar imediatamente
    accumulatedDX += dx * 1.8; // Aumentei um pouco a sensibilidade
    accumulatedDY += dy * 1.8;
}, { passive: false });
