...
//fechar pagina
app.post('/fechar_janela', async (req, res) => {
    await clipboardy.write("")
...
// position relative
let posAtual = { x: 500, y: 300 }; // inicial
mouse.move(new Point(posAtual.x, posAtual.y));
...
app.post('/mouse-mover-relativo', async (req, res) => {
    ...
    // 🔥 LIMITE TOTAL (AJUSTE AQUI)
    const MIN_X = -1920; // monitor da esquerda
    const MAX_X = 1920;  // monitor da direita
    const MIN_Y = 0;
    const MAX_Y = 1080;

    // aplica limites
    if (posAtual.x < MIN_X) posAtual.x = MIN_X;
    if (posAtual.x > MAX_X) posAtual.x = MAX_X;

    if (posAtual.y < MIN_Y) posAtual.y = MIN_Y;
    if (posAtual.y > MAX_Y) posAtual.y = MAX_Y;
    await mouse.move(new Point(posAtual.x, posAtual.y));
